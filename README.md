...
1) 로그인: 기존 아이디로 일반 사용자 / 기부 단체 / 외주 업체 옵션을 선택하여 로그인 가능하다.

#

2) 회원가입: 아이디 / 비밀번호 / 이름 / 주소 / 전화번호를 기입하여 회원가입 가능하다.

#

3) 메뉴 

-1. 기부 메뉴: 기부 단체들이 게시한 기부 목록을 게시판에서 확인하고, 자유롭게 기부에 참여할 수 있다.

-2. 선물 메뉴: 선물하고자 하는 사람의 아이디를 입력하여 그 사람의 선물 취향을 조회할 수 있다.

또, 선물 취향과 관련된 사이트를 확인해 볼 수 있다.

-3. 마이페이지 메뉴: 기존에 등록해둔 자신의 기부 취향과 선물 취향을 확인할 수 있으며, 자신이 참여한 기부 목록을 확인해 볼 수 있다.
          
또, 회원 탈퇴도 가능하다.
                   
-4. FAQ: 기부 또는 선물 등에 대해 발생할 수 있는 질문들에 대해 알아볼 수 있는 페이지이다. 

예를 들어, 'DㅓBㅔ풀기' 사이트에는 수많은 쇼핑몰 링크들이 존재하는데, 이 링크들 중 사용자들이 실제로 선물할 때 참고한 링크들을 알고싶다면 
이를 FAQ에서 확인 가능하다.

4) 지원페이지: 'DㅓBㅔ풀기' 사이트를 처음 접하는 사용자들이 보다 쉽게 사이트를 이용할 수 있도록 실행화면과 함께 사용 설명을 적어놓았다.
