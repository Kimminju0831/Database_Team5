...
메뉴는 총 7 가지로,
[ 1. 회원가입 / 2. 로그인 / 3. 로그아웃 / 4. 회원 정보 수정 / 5. 회원 탈퇴/ 6. 질문 / 7. 종료 ] 를 제공한다.

1) 회원 가입 : 사용자는 회원 가입 기능을 통해 db 에 유저의 이름, 주소, 전화번호, 아이디, 비밀번호 등을 추가할 수 있다.
2) 로그 인 : 사용자는 로그인 기능을 통해 db에 아이디와 비밀번호가
존재한다면 ‘현재 이용중인 유저’로 인식되고, 로그인할 수 있다. 이때
접속 중인 사용자에게는 회원가입과 로그인을 제외한 메뉴가 제공된다.
3) 로그 아웃 : 사용자는 로그 아웃 기능을 통해 현재 접속하지 않은
사용자로 로그아웃 할 수 있다.
4) 회원 정보 수정 : 사용자는 로그인한 이후에만 보이는 해당 기능을
통해 ‘본인의 아이디와 비밀번호’를 수정하는 것이 가능하다. 관리자가
아닌 일반 회원은 오직 본인의 정보만 수정할 수 있다. 이때 기존 DB에
저장된 아이디 / 비밀번호와 본인이 입력하는 아이디/ 비밀번호가
일치해야만 수정하는 것이 가능하다.
5) 회원 탈퇴 : 사용자는 로그인 한 이후에만 보이는 해당 기능을
이용하여 ‘본인의 유저 정보’를 db 에서 삭제할 수 있다. 관리자가 아닌
일반 회원은 본인의 탈퇴 유무만 결정할 수 있다.
6) 질문 : 실행시킬 수 있는 20개의 질문(query)들의 목록이 표시되고,
사용자는 이 20개의 질문들 중 자신이 실행시키고 싶은 질문을
실행하여 원하는 결과를 볼 수 있다. 여기서 질문으로 열람할 수 있는
데이터는 사용자의 권한(관리자, 일반 사용자)에 따라 다르다.
예를 들어, “몇 개 이상의 제품일 때, 배송 요금이 궁금하신가요?”라는
질문을 실행한다면, 자신이 보고싶은 질문 결과를 얻기 위해 개수에
해당하는 값을 입력해주면 콘솔창을 통해 원하는 결과를 볼 수 있다.
7) 종료 : 사용자는 종료 기능을 통해 해당 프로그램을 언제든지 종료할
수 있다.
3
2) 권한 설정
해당 프로그램의 권한은 총 두 가지로, 1) 일반 사용자 와 2) 관리자 로
나뉜다.
1) 일반 사용자에게 주어진 권한은 ‘회원 가입/ 본인의 정보 수정 /
본인의 정보 삭제 / 민감하지 않은 타인의 정보 열람’ 이다. 일반
사용자에게는 두 가지 상태의 메뉴를 제공한다.
1-1) 로그아웃 상태 : [ 1. 회원가입 / 2. 로그인 / 3. 종료 ] 를
제공한다.
1-2) 로그인 상태 : [ 1. 로그아웃 / 2. 회원 정보 수정 / 3. 회원 탈퇴
/ 4. 질문 / 5. 종료 ] 를 제공한다.
- 질문 메뉴의 경우, 민감하지 않은 타인의 정보만을 열람할 수
있어야 하므로, 관리자가 실행할 수 있는 총 20개의 질문들 중
7개의 질문들(1번, 2번, 10번, 14번, 18번, 19번, 20번)만 볼 수
있도록 한다.
2) 관리자에게 주어진 권한은 ‘유저 추가 / 유저 삭제 / 유저 정보 열람’
이다. 관리자에게는 두 가지 상태의 메뉴를 제공한다.
2-1) 로그아웃 상태 : [ 1. 회원가입 / 2. 로그인 / 3. 종료 ] 를
제공한다.
2-2) 로그인 상태 : [ 1. 로그아웃 / 2. 나의 정보 수정 / 3. 회원 관리
/ 4. 질문 / 5. 종료 ] 를 제공한다.
2-3) 로그인 상태 / 회원 관리 기능 실행 중 : [ 1. 회원 추가 /
2. 회원 수정 / 3. 회원 삭제 ] 를 제공한다.
